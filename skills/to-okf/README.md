...
Turn a codebase into an [OKF v0.1](https://github.com/GoogleCloudPlatform/knowledge-catalog/blob/main/okf/SPEC.md) bundle - a directory of small markdown concept docs that agents navigate through index files instead of re-reading the whole repo every session.
...
3. Proposes a bundle layout for approval, then writes it to `okf/`: one concept per file with YAML frontmatter (`type` required), `index.md` per directory for progressive disclosure, cross-links between concepts, and a `log.md` history.
...
- [OKF spec v0.1](https://github.com/GoogleCloudPlatform/knowledge-catalog/blob/main/okf/SPEC.md) - the authoritative format rules
